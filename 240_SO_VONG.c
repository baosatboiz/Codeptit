...
int check(char x[],char tmp[]){
	...
	for(i=0;i<strlen(tmp);i++) if(tmp[i]==x[0]) {test=1; b[j]=i; j++;}        //Tim vi tri cua chu so dau tien x[0],chu y: chu so dau tien trong so dang xet co the xuat hien 2 lan, neu ban chi xet vi tri dau tien tim duoc so giong x[0], co the sai bai toan vd: 0abx0 va x00ab
	                                                                          //Vi vay ta chi can tim dung vi tri so dau tien, bai toan coi nhu ket thuc, cac vi tri tim duoc so giong chu so dau tien x[0] ta luu vao mang b
	...
	i=b[p]+1;                             //Kiem tra cac chu so tu j+1 den so cuoi cung cua tmp                                                             
	   ...
	   if(i<strlen(tmp)) {for(k=1;k<=strlen(tmp)-b[p];k++) if(tmp[i]!=x[k]) { test2=1; break;} 
	   	...
	   	if(test2==0) {test3=1; j=b[p]; break;}    }      //neu trong vong for,so dang xet thoa man, test2=0-> test3=1 de kh bi return                         
	if(test3==0) return 0;             //neu het for test3!=1 tuc la test2 luon bang 1 sau moi vong lap, hay noi cach khac cac so deu khong thoa mãn,return
